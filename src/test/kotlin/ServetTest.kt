...
class ServetTest(
    private var mongoDriver: RxMongoDriver, private var server: Server
) {
    companion object {
        ...
        private const val NEW_PRODUCT_RESPONSE =
            "New product:\n" + "Product: {\n" + "\tid: 42,\n" + "\tname: Test,\n" + "\tRUB: 80,\n" + "\tEUR: 1,\n" + "\tUSD: 2\n" + "}"

        private const val NEW_USER_RESPONSE =
            "New user:\n" + "User: {\n" + "\tid: 101,\n" + "\tname: Test,\n" + "\tcurrency: USD\n" + "}"
    }
    ...
    fun testAddProduct() {
        ...
        Assert.assertEquals(
            NEW_PRODUCT_RESPONSE, (res as ScalarSynchronousObservable<*>).get().toString()
        )
    }
